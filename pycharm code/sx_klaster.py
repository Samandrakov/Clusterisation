...
df = pd.read_excel('gor_prom.xlsx')

df = df.to_excel('res1(gor)_bez_chistki.xlsx')

df = pd.read_excel('res1(gor)_bez_chistki.xlsx')

df = df.drop(['Ranking','Company','Sector',"Country","Ventas","neto","oper","Empl","Roe"],  axis=1)
df.dropna(axis=0, inplace=True)
df = df.to_excel('res1(gor).xlsx')

df = pd.read_excel('res1(gor).xlsx')
print(df.head())
